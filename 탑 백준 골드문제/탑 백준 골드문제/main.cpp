...
//1.5 초	128 MB	91736	32581	22042	33.778 %
//문제
//KOI 통신연구소는 레이저를 이용한 새로운 비밀 통신 시스템 개발을 위한 실험을 하고 있다.실험을 위하여 일직선 위에 N개의 높이가 서로 다른 탑을 수평 직선의 왼쪽부터 오른쪽 방향으로 차례로 세우고, 
...
int main()
{
	...
	string numinfo;
	cin >> size;
	int* reach_info = new int[size];
	if (reach_info == nullptr) { return - 1; }
	else {
	...
	stringstream io(numinfo);
	int num;
	int removed_top = 0;
	int idx = 0;
	while (io >> num && size > idx) {
		...
		if (topinfo.empty()) {
			...
			int ins = removed_top;
			while ( !topinfo.empty() && topinfo.top() < num) {
				topinfo.pop();
				ins += 1;
			}
			...
			if (topinfo.empty()) {
				...
				reach_info[idx] = (int)topinfo.size() + removed_top;
			}
			removed_top = ins;
			topinfo.push(num);
